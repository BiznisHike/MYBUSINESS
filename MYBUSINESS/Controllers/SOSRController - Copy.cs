...
using MYBUSINESS.ActionFilters;
    ...
    public class SOSRController : Controller
    {
...

        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (previousBill.Balance == sO.Balance && previousBill.BillAmount == sO.BillAmount && previousBill.BillPaid == sO.BillPaid
                    && previousBill.CustomerId == sO.CustomerId && previousBill.Discount == sO.Discount && previousBill.PrevBalance == sO.PrevBalance
                    ...
                    //&& span.TotalSeconds < 60)
                {
                    ...
                    SOId = string.Join("-", ASCIIEncoding.ASCII.GetBytes(Encryption.Encrypt(previousBill.Id, "BZNS")));
        ...
        public FileContentResult PrintSO4()
        {
            //var paramValue = RouteData.Values["SOIDEnc"].ToString();
            string id = Request.QueryString["SOIDEnc"].ToString();
            id = Decode(id);
            // Variables
            Warning[] warnings;
            string[] streamIds;
